...
import {
  compileVisualFilterPredicate,
  compileVisualFiltersWhereClause,
  quoteIdentifier,
...
import {
  analyticalQueryClient,
  analyticalQueryKey,
} from "./analyticalQueryClient";
...
const tableNameForLayer = (layerId: string) =>
  `visual_layer_${layerId.replace(/[^a-zA-Z0-9_]/g, "_")}`;
...
export const analyticsTableForLayer = async (layer: AnalyticsLayer) => {
  ...
  return registerLayerForAnalytics({
    id: layer.id,
    geojson: layer.geojson || { type: "FeatureCollection", features: [] },
  });
};

export const queryLayerFeatureDetails = async (
  ...
  if (!layer.source || layer.source.kind === "legacy-geojson") {
    const feature = layer.geojson?.features.find(
      (item) =>
        String(item.properties?.[FEATURE_ID_PROPERTY] ?? item.id ?? "") ===
        featureId,
    );
...
export const queryLayerSelectionBounds = async (
  ...
  if (!layer.source || layer.source.kind === "legacy-geojson") {
    const coordinates = (layer.geojson?.features || [])
      .filter((feature) =>
        selected.has(
          String(feature.properties?.[FEATURE_ID_PROPERTY] ?? feature.id ?? ""),
        ),
      )
...
const layerSignature = (layer: {
  ...
  const firstId =
    features[0]?.properties?.[FEATURE_ID_PROPERTY] ?? features[0]?.id ?? "";
  const lastId =
    features.at(-1)?.properties?.[FEATURE_ID_PROPERTY] ??
    features.at(-1)?.id ??
    "";
  return `${features.length}:${String(firstId)}:${String(lastId)}`;
};

export const registerLayerForAnalytics = async (layer: {
  id: string;
  geojson: GeoJSON.FeatureCollection;
}) => {
...
export const clearLayerAnalyticsCache = (layerId?: string) => {
  ...
  [...kpiQueryCache.keys()]
    ...
    .forEach((key) => kpiQueryCache.delete(key));
  [...datasetProfileCache.keys()]
    .filter((key) => key.startsWith(`${layerId}:`))
    .forEach((key) => datasetProfileCache.delete(key));
...
export const queryLayerFeatureIds = async ({
  ...
  const tableName = await analyticsTableForLayer(layer);
  const relation = buildComputedRelation(`"${tableName}"`, computedFields);
  const whereClause = layerSearchWhereClause(
    layer,
    filters,
    search,
    computedFields,
...
export const materializeLayerSelection = async ({
  ...
  await duckdbService.registerJsonRows(
    idTableName,
  ...
  try {
    ...
    await duckdbService
      .query(`DROP TABLE IF EXISTS "${idTableName}";`)
      .catch(() => undefined);
  }
  const source = await duckdbService.prepareLayerSource(safeTableName, {
    kind: "duckdb-query",
    originalTableName: safeTableName,
  });
  if (!source) return null;
  return {
    source,
    featureCount: await duckdbService.getTableFeatureCount(safeTableName),
  };
};
...
const kpiSourceVersion = (layer: AnalyticsLayer) => {
  ...
  if (layer.geojson)
    return layerSignature({ id: layer.id, geojson: layer.geojson });
  return "unknown";
};
...
export const queryLayerKpi = async ({
  ...
  const promise = analyticalQueryClient
    .run(
      {
        key: analyticalQueryKey("kpi", {
          datasetId: layer.id,
          sourceVersion: kpiSourceVersion(layer),
          filters,
          selectedFeatureIds,
          spec,
        }),
      ...
      async () => {
        ...
        if (spec.comparison === "previous-period") {
          const window = filters.find(
            (filter): filter is Extract<VisualFilter, { kind: "temporal" }> =>
              filter.kind === "temporal" && Boolean(filter.start && filter.end),
          );
          ...
          if (
            ...
            const otherFilters = filters
              .filter((filter) => filter !== window)
              .map(compileVisualFilterPredicate)
              .filter((item): item is string => Boolean(item));
            const field = `TRY_CAST(${quoteIdentifier(window.field)} AS TIMESTAMP)`;
            comparisonPredicate = [
              ...otherFilters,
              `${field} >= TRY_CAST(${sqlString(new Date(previousStart).toISOString())} AS TIMESTAMP)`,
              `${field} <= TRY_CAST(${sqlString(new Date(previousEnd).toISOString())} AS TIMESTAMP)`,
            ].join(" AND ");
          } else {
            comparisonNote =
              "Apply a bounded temporal filter to compare with the preceding period.";
          }
        } else if (spec.comparison === "cohort") {
          if (selectedFeatureIds.length) {
            comparisonPredicate = selectedWhereClause(
              selectedFeatureIds.slice(0, 25_000),
              featureIdColumnForLayer(layer),
            ).replace(/^WHERE\s+/, "");
          } else {
            comparisonNote =
              "Select records to use the current selection as a comparison cohort.";
          }
        }
        const comparisonExpression =
          spec.comparison === "total"
       ...
       FROM ${table};`,
        ...
        const comparisonValue =
          spec.comparison === "none"
            ? null
            : finiteOrNull(row.comparison_value);
        const comparisonAvailable =
          spec.comparison !== "none" && comparisonValue !== null;
        const delta =
          comparisonAvailable &&
          value !== null &&
          ...
          comparisonValue !== 0
            ...
            : null;
        if (comparisonAvailable && comparisonValue === 0)
          comparisonNote =
            "Delta is unavailable because the comparison value is zero.";
        return {
          specId: spec.id,
          value,
          comparisonValue,
    ...
    .run(
      ...
      async () => {
       ...
       FROM ${table};`,
        ...
        const comparisonValue =
          spec.comparison === "none" ? null : finite(row.comparison_value);
        const comparisonAvailable =
          spec.comparison !== "none" && comparisonValue !== null;
        const delta =
          comparisonAvailable && value !== null && comparisonValue !== 0
            ? (value - comparisonValue!) / Math.abs(comparisonValue!)
            : null;
        if (comparisonAvailable && comparisonValue === 0)
          comparisonNote =
...
export const queryLayerDatasetProfile = async (
  ...
  const promise = analyticalQueryClient
    .run(
      {
        key: analyticalQueryKey("dataset-profile", {
          datasetId: layer.id,
          sourceVersion: kpiSourceVersion(layer),
        }),
        datasetId: layer.id,
      ...
      async () => {
        ...
        if (sampledFeatures && sampledValid < sampledFeatures)
          issues.push({
            id: "missing-geometry",
            severity: "warning",
        ...
        const declaredCrs =
          ...
          layer.source.kind === "duckdb-query"
            ? layer.source.crs
            : undefined;
        const crsConfidence: DatasetGeometryProfile["crsConfidence"] =
          layer.source.kind === "duckdb-table" ||
          layer.source.kind === "duckdb-query"
...
const aggregationExpression = (
  ...
  const filterSuffix = filterPredicate
    ? ` FILTER (WHERE ${filterPredicate})`
    : "";
  if (chart.aggregation === "count" || !chart.measureField)
    return `COUNT(*)${filterSuffix}`;

  const field = `TRY_CAST(${quoteIdentifier(chart.measureField)} AS DOUBLE)`;
  if (chart.aggregation === "sum")
    return `COALESCE(SUM(${field})${filterSuffix}, 0)`;
  if (chart.aggregation === "avg")
    return `COALESCE(AVG(${field})${filterSuffix}, 0)`;
  if (chart.aggregation === "min")
    return `COALESCE(MIN(${field})${filterSuffix}, 0)`;
    ...
    return Array.from(
      ...
      (_, index) =>
        palette[
          Math.min(
            palette.length - 1,
            Math.floor((index / Math.max(1, count - 1)) * (palette.length - 1)),
          )
        ],
    );
  }

  const sequential = getPalette(
    chart.paletteId,
    CATEGORICAL_PALETTE_META,
  ).colors;
  const source =
    chart.paletteId === "categorical" ? CATEGORICAL_PALETTE : sequential;
  return Array.from(
    { length: count },
...
const tableColumnNames = async (tableName: string) => {
  ...
  return new Set(
    normalizeRows(schema.toArray()).map((row) => String(row.name || "")),
  );
};

const chartTableForLayer = async (
  layer: AnalyticsLayer,
  chart: VisualChartSpec,
) => {
  const tableName = await analyticsTableForLayer(layer);
  ...
  if (
    ...
    try {
      const tileColumns = await tableColumnNames(
        layer.source.tileSource.tableName,
      );
      const tileHasRequiredFields = requiredColumns.every((column) =>
        tileColumns.has(column),
      );
...
export const queryLayerChart = async ({
  ...
  const { tableName, featureIdExpression } = await chartTableForLayer(
    layer,
    chart,
  );
  return analyticalQueryClient.run(
    {
      key: analyticalQueryKey("chart", {
        datasetId: layer.id,
        sourceVersion: kpiSourceVersion(layer),
        filters,
        chart,
  ...
  analyticalQueryClient.run(
    {
      ...
      datasetId:
        chart.source && "datasetId" in chart.source
          ? chart.source.datasetId
          : `table:${tableName}`,
    },
    () =>
      runChartQuery({
        tableName,
        featureIdExpression: rowIdColumn
...
const runTemporalChartQuery = async ({
  ...
  const [totalResult, filteredResult, extentResult] = await Promise.all([
    ...
    duckdbService.query(
      `SELECT COUNT(*) AS row_count FROM ${table} ${whereClause};`,
    ),
    duckdbService.query(
      `SELECT MIN(${timestamp}) AS min_date, MAX(${timestamp}) AS max_date FROM ${table};`,
    ),
  ]);
  const totalRows = Number(
    normalizeRows(totalResult.toArray())[0]?.row_count ?? 0,
  );
  const filteredRows = Number(
    normalizeRows(filteredResult.toArray())[0]?.row_count ?? 0,
  );
  ...
  if (chart.seriesField) {
    ...
    const ranked = normalizeRows(topResult.toArray()).map((row) =>
      String(row.series),
    );
  ...
  const aggregate = contextPredicate
    ? aggregationExpression(chart, contextPredicate)
    : totalAggregate;
  const filteredCount = contextPredicate
    ? `COUNT(*) FILTER (WHERE ${contextPredicate})`
    : "COUNT(*)";
  const result = await duckdbService.query(
      ...
      points: buckets.map((bucket) => {
        ...
        return {
          ...
          value: hasObservation
            ? Number(row?.aggregate_value ?? row?.count_value ?? 0)
            : null,
...
export const queryLayerTemporalChart = async ({
  ...
  return analyticalQueryClient.run(
    {
      key: analyticalQueryKey("temporal-chart", {
        datasetId: layer.id,
        sourceVersion: kpiSourceVersion(layer),
        filters,
        chart,
...
export const queryChartFacetValues = async ({
  ...
  const resolved =
    tableName ?? (layer ? await analyticsTableForLayer(layer) : null);
  if (!resolved) return [];
...
const runChartQuery = async ({
  ...
  const aggregate = contextPredicate
    ? aggregationExpression(chart, contextPredicate)
    : totalAggregate;
  const filteredCountExpr = contextPredicate
    ? `COUNT(*) FILTER (WHERE ${contextPredicate})`
    : "COUNT(*)";
  const featureIdFilterSuffix = contextPredicate
    ? ` FILTER (WHERE ${contextPredicate})`
    : "";

  const combinedPredicates = [whereClause.replace(/^WHERE\s+/, ""), facetWhere]
    .filter(Boolean)
    .join(" AND ");
  const [totalResult, filteredResult] = await Promise.all([
    duckdbService.query(
      `SELECT COUNT(*) AS row_count FROM ${table} ${facetWhere ? `WHERE ${facetWhere}` : ""};`,
    ),
    duckdbService.query(
      `SELECT COUNT(*) AS row_count FROM ${table} ${combinedPredicates ? `WHERE ${combinedPredicates}` : ""};`,
    ),
  ]);
  const totalRaw = normalizeRows(totalResult.toArray())[0] || {};
...
export const queryLayerScatter = async ({
  ...
  const { tableName } = await chartTableForLayer(layer, {
    ...chart,
    aggregation: "avg",
  });
  return analyticalQueryClient.run(
    {
      key: analyticalQueryKey("scatter-chart", {
        datasetId: layer.id,
        sourceVersion: kpiSourceVersion(layer),
        filters,
        chart,
...
const runScatterQuery = async ({
  ...
  const contextPredicate = filters
    .filter(
      (filter) =>
        filter.field !== chart.dimensionField &&
        filter.field !== chart.measureField,
    )
    .map(compileVisualFilterPredicate)
    .filter((item): item is string => Boolean(item))
    .join(" AND ");
  ...
  const sampleClause = sampled
    ? ` USING SAMPLE reservoir(${SCATTER_MAX_POINTS} ROWS) REPEATABLE (7)`
    : "";
  const result = await duckdbService.query(
...
export const queryLayerTemporalRange = async ({
  ...
  const minDate =
    typeof raw.min_date === "object" &&
    raw.min_date &&
    typeof (raw.min_date as any).toISOString === "function"
      ? (raw.min_date as Date).toISOString().slice(0, 10)
      : String(raw.min_date).slice(0, 10);
  const maxDate =
    typeof raw.max_date === "object" &&
    raw.max_date &&
    typeof (raw.max_date as any).toISOString === "function"
...
const selectedWhereClause = (
  selectedFeatureIds: string[],
  field = FEATURE_ID_PROPERTY,
) => {
  if (!selectedFeatureIds.length) return "";
  const values = selectedFeatureIds
    .map((id) => `'${id.replace(/'/g, "''")}'`)
    .join(", ");
...
export const queryLayerSummary = async ({
  ...
  const activePredicate =
    filters
      ...
      .filter((item): item is string => Boolean(item))
      .join(" AND ") || "TRUE";
  const selectionWhere = selectedWhereClause(
    selectedFeatureIds.slice(0, 25_000),
    featureIdColumnForLayer(layer),
  );
  ...
  const numericMetrics = await Promise.all(
    numericColumns.map(async (field) => {
      const q = quoteIdentifier(field);
      const numeric = `TRY_CAST(${q} AS DOUBLE)`;
      const aggregate = (name: string, expression: string, predicate: string) =>
        `${expression} FILTER (WHERE ${predicate}) AS ${name}`;
      const result = await duckdbService.query(
        `SELECT ${["selected", "active", "total"]
          .flatMap((scope) => {
            const predicate =
              scope === "selected"
                ? selectedPredicate
  ...
  const categoryBreakdowns = await Promise.all(
    categoryColumns.map(async (field) => {
      const q = quoteIdentifier(field);
      const result = await duckdbService.query(
...
const safeCohortTableName = (value: string) => {
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(value))
    throw new Error(
      "The saved selection cohort has an invalid table reference.",
    );
  return value;
};
...
export const queryCohortComparison = async ({
  ...
  if (
    cohortA.datasetId !== layer.id ||
    (cohortB && cohortB.datasetId !== layer.id)
  )
    throw new Error("Both cohorts must belong to the selected dataset.");
  if (!cohortB && !compareToRemainder)
    throw new Error("Choose a second cohort or compare with the remainder.");
  const tableName = await analyticsTableForLayer(layer);
  const table = `"${tableName.replace(/"/g, '""')}"`;
  ...
  const activePredicate =
    remainderFilters
      ...
      .filter((item): item is string => Boolean(item))
      .join(" AND ") || "TRUE";
  const predicateB = compareToRemainder
    ? `(${activePredicate}) AND NOT (${predicateA})`
    : cohortPredicate(cohortB!, idColumn);
  const countResult = await duckdbService.query(
    `SELECT COUNT(*) AS total_rows,
  ...
  const numericFields = metadata.fields
    ...
    .slice(0, 4);
  const categoricalFields = metadata.fields
    .filter((field) => ["categorical", "boolean"].includes(field.semanticType))
    .slice(0, 3);
  const temporalField = metadata.fields.find(
    (field) => field.semanticType === "temporal",
  );

  const numeric = await Promise.all(
    numericFields.map(async ({ name: field }) => {
      ...
      const pooledSd =
        aSd !== null && bSd !== null
          ? Math.sqrt((aSd * aSd + bSd * bSd) / 2)
          : null;
      const effectSize =
        aMean !== null && bMean !== null && pooledSd && pooledSd > 0
          ? (aMean - bMean) / pooledSd
          : null;
          ...
          : await (async () => {
              ...
              const expressions = Array.from(
                ...
                (_, index) => {
                  ...
                  const upper =
                    index === binCount - 1 ? max : min + width * (index + 1);
                  const range =
                    index === binCount - 1
              ...
              const result = await duckdbService.query(
                `SELECT ${expressions.join(", ")} FROM ${table} AS base;`,
              );
              const row = normalizeRows(result.toArray())[0] || {};
              return Array.from({ length: binCount }, (_, index) => {
                ...
                const upper =
                  index === binCount - 1 ? max : min + width * (index + 1);
                return {
                  label: `${lower.toLocaleString(undefined, { maximumFractionDigits: 2 })}–${upper.toLocaleString(undefined, { maximumFractionDigits: 2 })}`,
                  aCount: Number(row[`a_bin_${index}`] ?? 0),
                  bCount: Number(row[`b_bin_${index}`] ?? 0),
                };
              });
            })();
      return {
        ...
        aCount,
        bCount,
        aMissing: Math.max(0, aRows - aCount),
        bMissing: Math.max(0, bRows - bCount),
        aMean,
        bMean,
        effectSize,
        bins,
  ...
  const categorical = await Promise.all(
    categoricalFields.map(async ({ name: field }) => {
  ...
  if (temporalField) {
    ...
    temporal = {
      ...
      grain: "month",
      points: normalizeRows(result.toArray()).map((row) => ({
        period: String(row.period),
        aCount: Number(row.a_count ?? 0),
        bCount: Number(row.b_count ?? 0),
      })),
    };
  }
  ...
  return {
    ...
    missingValueNote:
      "Numeric means and effect sizes exclude missing or non-numeric values; missing counts are reported per cohort. Category shares include missing values as “∅ Missing”.",
    numeric,
    categorical,
    temporal,
  };
...
export const queryLayerFieldProfile = async ({
  ...
  if (
    ...
    const values = normalizeRows(valuesResult.toArray())
    ...
    values.forEach((value) => {
      const index =
        max === min
          ? 0
          : Math.min(binCount - 1, Math.floor((value - min) / (width || 1)));
      bins[index].count += 1;
    });
...
export const explainLayerSelection = async ({
  ...
  const probeResult = await duckdbService.query(
    `SELECT COUNT(*) FILTER (WHERE ${selPredicate}) AS sel_count,
            ...
            ${columns
              .map(
                (column, index) =>
                  `COUNT(TRY_CAST(${quoteIdentifier(column)} AS DOUBLE)) AS p${index}_num, COUNT(${quoteIdentifier(column)}) AS p${index}_all`,
              )
              .join(", ")}
     FROM ${table};`,
  );
  ...
  columns.forEach((column, index) => {
    ...
    if (
      nonNull > 0 &&
      numeric >= nonNull * 0.8 &&
      numericColumns.length < EXPLAIN_MAX_NUMERIC_FIELDS
    ) {
      numericColumns.push(column);
    } else if (
      nonNull > 0 &&
      categoricalColumns.length < EXPLAIN_MAX_CATEGORY_FIELDS
    ) {
  ...
  for (const column of categoricalColumns) {
    ...
    const score =
      categories.reduce(
        (sum, item) => sum + Math.abs(item.selectedShare - item.restShare),
        0,
      ) / 2;
    categories.sort(
      (a, b) =>
        Math.abs(b.selectedShare - b.restShare) -
        Math.abs(a.selectedShare - a.restShare),
    );
    fields.push({
      kind: "categorical",
      field: column,
...
export const describeChartTable = async (
  tableName: string,
