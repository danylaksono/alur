...
import {
  buildLegendItemFilter,
  toggleFilterIn,
  visualFilterKey,
} from "../../utils/legendFilter";
      ...
      {!panelCollapsed && (
        <div className="space-y-2">
          {legends.slice(0, 6).map(({ layerId, layerName, legend }) => {
            ...
            const isFilterable =
              legend.kind === "choropleth" ||
              legend.kind === "categorical" ||
              legend.kind === "extrusion";
            const isBivariate = legend.kind === "bivariate";
            const isCollapsed = Boolean(collapsed[layerId]);
            const query = searches[layerId] || "";
            const matches = legend.items.filter((item) =>
              item.label
                .toLocaleLowerCase()
                .includes(query.toLocaleLowerCase()),
            );
            const visibleItems =
              query || expandedItems[layerId]
                ? matches
                : matches.slice(0, MAX_ITEMS);
            const gridItems = isBivariate
              ? legend.items.filter(
                ...
                {!isCollapsed && (
                  <div className="mt-1.5 space-y-1.5">
                    <div className="flex items-start justify-between gap-2 text-[9px] text-slate-400">
                      ...
                      {method && (
                        <span className="shrink-0 capitalize">{method}</span>
                      )}
                    </div>
                    ...
                    {isBivariate ? (
                          ...
                          {[2, 1, 0].flatMap((row) =>
                            [0, 1, 2].map((column) => {
                              ...
                              return (
                                <span
                                  key={`${row}-${column}`}
                                  className="h-4 w-4 border border-white/50"
                                  style={{
                      ...
                      <div className="space-y-0.5">
                        {visibleItems.map((item, index) => {
                          const itemFilter = buildLegendItemFilter(
                            legend,
                            item,
                          );
                          const isActive = activeKeys.has(
                            visualFilterKey(itemFilter),
                          );
                          const row = (
                            <>
                              <span
                          ...
                          return isFilterable ? (
                            <button
                              ...
                              onClick={() =>
                                setLayerFilters(
                                  layerId,
                                  toggleFilterIn(filters, itemFilter),
                                )
                              }
                        ...
                        {!query && legend.items.length > MAX_ITEMS && (
                          <button
                            type="button"
                            onClick={() =>
                          ...
                          >
                            {expandedItems[layerId]
                              ? "Show less"
                              : `Show ${legend.items.length - MAX_ITEMS} more`}
                          </button>
                        )}
                      </div>
                    )}
