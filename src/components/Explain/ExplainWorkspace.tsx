...
import { createStory, parseStory } from '../../services/storyService';
import { evaluateExplainDocument } from '../../utils/explainEvidence';
import { variantLineageSnapshot } from '../../utils/variantLineage';

/** Current lineage, ready to freeze onto a card. */
const lineageSnapshot = () => {
  const state = useStore.getState();
  return variantLineageSnapshot(
    state.visualAnalytics.variants,
    (variant) => Boolean(variant.workflowOutputDatasetId && state.datasetRegistry[variant.workflowOutputDatasetId]),
    Date.now(),
  );
};
...
const ComparisonEvidence = ({ result, card, presenting }: { result: ComparisonResult; card: ExplainCard; presenting: boolean }) => {
  ...
  return <div className="space-y-4">{result.summaries.map((summary) => <div key={summary.measureId}><h4 className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{spec?.measures.find((item) => item.id === summary.measureId)?.label || summary.measureId}</h4><div className="mt-2 space-y-2">{summary.values.map((value) => { const operand = spec?.operands.find((item) => item.id === value.operandId); return <div key={value.operandId} className="grid grid-cols-[100px_1fr_64px] items-center gap-2 text-[11px]"><span className="truncate font-semibold text-slate-600">{operand?.label || value.operandId}</span><div className="h-2 rounded-full bg-slate-100"><div className="h-full rounded-full" style={{ width: `${Math.max(2, ((value.value || 0) / max) * 100)}%`, backgroundColor: operand?.colour || '#64748b' }} /></div><span className="text-right tabular-nums">{value.value?.toLocaleString(undefined, { maximumFractionDigits: 2 }) ?? '—'}</span></div>; })}</div></div>)}</div>;
};
...
const CardContent = ({ card, presenting = false }: { card: ExplainCard; presenting?: boolean }) => {
  ...
  if (card.kind === 'table') {
    ...
    return <div><div className="flex items-center gap-2"><Database className="h-4 w-4 text-teal-600" /><h3 className="text-sm font-bold text-slate-800">{card.title || dataset?.name || 'Table preview'}</h3></div>{preview.length ? <div className="mt-3 overflow-x-auto rounded-lg border border-slate-200"><table className="w-full text-left text-[11px]"><thead className="bg-slate-50 text-slate-500"><tr>{columns.map((column) => <th key={column} className="whitespace-nowrap px-2 py-1.5 font-bold">{column}</th>)}</tr></thead><tbody>{preview.slice(0, 10).map((row, index) => <tr key={index} className="border-t border-slate-100">{columns.map((column) => <td key={column} className="max-w-40 truncate px-2 py-1.5 text-slate-600">{String(row[column] ?? '—')}</td>)}</tr>)}</tbody></table></div> : <p className="mt-4 text-xs text-slate-500">No bounded rows were captured.</p>}</div>;
  }
  if (card.kind === 'finding') return <div><div className="flex flex-wrap items-center gap-2"><NotebookPen className="h-5 w-5 text-amber-500" /><span className={cn('rounded-full px-2 py-1 text-[11px] font-bold uppercase tracking-wide', card.conclusionStatus === 'supported' ? 'bg-emerald-50 text-emerald-700' : card.conclusionStatus === 'contested' ? 'bg-rose-50 text-rose-700' : 'bg-slate-100 text-slate-600')}>{card.conclusionStatus || 'draft'}</span><span className="text-[11px] font-semibold capitalize text-slate-500">{card.confidence || 'tentative'} confidence</span></div><h3 className="mt-3 text-base font-extrabold leading-6 text-slate-900">{card.claim || 'Select this finding to write its claim.'}</h3>{card.interpretation && <p className="mt-3 text-xs leading-5 text-slate-600">{card.interpretation}</p>}{card.caveat && <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-[11px] leading-4 text-amber-900"><strong>Caveat:</strong> {card.caveat}</p>}<p className="mt-3 text-[11px] font-semibold text-slate-500">{card.evidenceLinks?.length || 0} linked evidence item{card.evidenceLinks?.length === 1 ? '' : 's'}</p></div>;
...
const SortableCard = ({ card, presenting, selected, onSelect }: { card: ExplainCard; presenting: boolean; selected: boolean; onSelect: () => void }) => {
  ...
  return <article id={`explain-card-${card.id}`} ref={setNodeRef} onClick={onSelect} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn('relative overflow-hidden rounded-xl border bg-white p-4 shadow-sm outline-none transition-colors', widthClass[card.width], presenting ? 'min-h-0' : heightClass[card.height], !presenting && 'cursor-pointer pt-12', presenting && card.behaviour === 'live' && 'pt-9', selected ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200', isDragging && 'z-20 opacity-45 shadow-xl')} tabIndex={presenting ? -1 : 0} onFocus={onSelect} aria-label={`${card.title || card.claim || card.kind} card`}>
    {!presenting && <div className="absolute right-2 top-2 z-10 flex rounded-md border border-slate-100 bg-white/95 p-0.5 shadow-sm"><button type="button" {...attributes} {...listeners} className="pressable cursor-grab rounded p-1.5 text-slate-600 hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-blue-500" aria-label={`Drag to reorder ${card.title || card.kind}`}><GripVertical className="h-3.5 w-3.5" /></button>{card.provenance && <button type="button" onClick={() => void refresh()} disabled={sourceMissing} className="pressable rounded p-1.5 text-slate-600 hover:bg-slate-50 disabled:opacity-30" aria-label="Refresh frozen evidence"><RefreshCw className="h-3.5 w-3.5" /></button>}<button type="button" onClick={() => update(card.id, { behaviour: card.behaviour === 'frozen' ? 'live' : 'frozen' })} className="pressable rounded px-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50" aria-label={`Make card ${card.behaviour === 'frozen' ? 'live' : 'frozen'}`}>{card.behaviour === 'frozen' ? 'Live' : 'Freeze'}</button></div>}
    {(sourceMissing || sourceChanged) && <div className={cn('mb-3 mr-28 rounded-md px-2 py-1 text-[11px] font-bold', sourceMissing ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700')}>{sourceMissing ? 'Source missing — frozen evidence retained' : 'Source changed — refresh is optional'}</div>}
    ...
    {/* What the scenario behind this evidence took for granted. Captured when
        the card was pinned, so it stays true to the number above it. */}
    {(card.provenance?.assumptions?.length || 0) > 0 && <div className="mt-3 border-t border-slate-100 pt-2"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Assumed</p><ul className="mt-1 space-y-0.5">{card.provenance!.assumptions!.map((assumption) => <li key={assumption} className="text-[11px] leading-4 text-slate-500">· {assumption}</li>)}</ul></div>}
    {card.caption && <p className="mt-3 border-t border-slate-100 pt-2 text-[11px] leading-4 text-slate-600">{card.caption}</p>}
    {(!presenting || card.behaviour === 'live') && <div className="absolute left-4 top-3 flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide text-slate-600">{card.behaviour === 'live' ? <><RefreshCw className="h-2.5 w-2.5" /> Live evidence</> : 'Frozen evidence'}</div>}
  </article>;
};
...
export const ExplainWorkspace = () => {
  ...
  // Your working explanation is snapshotted for the comparison, so both sides
  // are frozen accounts and neither can be edited out from under the other.
  const handleCompareImport = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = '';
    if (!file) return;
    try {
      compareStories(createStory(), parseStory(await file.text()));
    } catch (error: any) {
  ...
  const add = async (kind: ExplainCardKind, sectionId = 'evidence') => {
    ...
    const card: ExplainCard = { id: `explain-${kind}-${Date.now()}`, sectionId, kind, datasetId: kind === 'table' ? dataset?.id : undefined, title: kind === 'finding' ? 'Finding' : kind === 'note' ? 'Note' : kind === 'lineage' ? 'Scenario lineage' : kind === 'account' ? 'How this came about' : undefined, width: kind === 'comparison' || kind === 'map' || kind === 'account' ? 12 : 6, height: kind === 'finding' || kind === 'lineage' || kind === 'account' ? 'standard' : 'compact', behaviour: 'frozen', provenance: { capturedAt: Date.now(), datasetIds: dataset && kind === 'table' ? [dataset.id] : [], sourceVersions: dataset ? { [dataset.id]: dataset.sourceUpdatedAt } : {}, filtersByDataset: {}, caveats: [] } };
    ...
    // A story is read where none of this state exists, so the lineage has to
    // travel inside the card rather than be looked up.
    if (kind === 'lineage') card.frozenValues = lineageSnapshot();
    // Same reason, and the account is what answers Refine's question for a
    // reader who never had the project open. Frozen at the scope on screen, so
    // pinning while one enquiry is open does not silently publish the others.
    if (kind === 'account') {
      const { provenanceEvents, visualAnalytics } = useStore.getState();
      card.frozenValues = visualAnalytics.activeSessionId ? eventsForSession(provenanceEvents, visualAnalytics.activeSessionId) : provenanceEvents;
    }
  ...
  return <div className="relative flex min-h-0 flex-1 overflow-hidden bg-slate-50">
    ...
    <main className={cn('min-h-0 min-w-0 flex-1 overflow-y-auto bg-slate-50 print:overflow-visible print:bg-white', presenting && 'bg-white')}>
      <div className="mx-auto max-w-[1320px] p-4 md:p-7 print:max-w-none print:p-0">
        <header className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm print:border-0 print:shadow-none"><div className="flex flex-wrap items-start gap-3"><div className="min-w-52 flex-1"><input value={explain.title} readOnly={presenting} onChange={(event) => setTitle(event.target.value)} className="w-full bg-transparent text-xl font-black text-slate-900 outline-none" aria-label="Explanation title" />{presenting ? explain.summary && <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">{explain.summary}</p> : <textarea value={explain.summary || ''} onChange={(event) => updateDocument({ summary: event.target.value })} placeholder="Executive summary or central analytical question…" className="mt-1 min-h-8 w-full resize-none bg-transparent text-xs leading-5 text-slate-600 outline-none" aria-label="Explanation summary" />}{explain.audience && presenting && <p className="mt-1 text-[11px] font-semibold text-slate-500">For {explain.audience}</p>}</div><div className="flex flex-wrap gap-2">{!presenting && <><button type="button" onClick={() => { setOutlineOpen(true); setInspectorOpen(false); }} className="pressable flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-3 text-[11px] font-bold text-slate-700 xl:hidden"><PanelLeft className="h-3.5 w-3.5" /> Outline</button>{selectedCard && <button type="button" onClick={() => { setInspectorOpen(true); setOutlineOpen(false); }} className="pressable flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-3 text-[11px] font-bold text-slate-700 xl:hidden"><SlidersHorizontal className="h-3.5 w-3.5" /> Inspect</button>}<div className="relative"><button type="button" onClick={() => setAddMenu(!addMenu)} className="pressable flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-3 text-[11px] font-bold text-slate-700"><Plus className="h-3.5 w-3.5" /> Add</button>{addMenu && <div className="absolute right-0 top-10 z-30 grid w-56 rounded-lg border border-slate-200 bg-white p-1 shadow-xl">{(['finding', 'note', 'table', 'lineage', 'account'] as const).map((kind) => <button key={kind} type="button" onClick={() => void add(kind)} disabled={kind === 'table' && !datasets.length} className="pressable rounded px-3 py-2 text-left text-[11px] font-semibold capitalize text-slate-700 hover:bg-slate-50 disabled:opacity-40">Add {kind}</button>)}<p className="border-t border-slate-100 px-3 py-2 text-[11px] leading-4 text-slate-500">Maps, charts, KPIs and comparison views are pinned from their analytical workspace so their current context can be captured.</p></div>}</div></>}{!presenting && <><button type="button" onClick={() => compareInputRef.current?.click()} className="pressable flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-3 text-[11px] font-bold text-slate-700" title="Compare your findings with a story someone shared"><GitCompareArrows className="h-3.5 w-3.5" /> Compare</button><input ref={compareInputRef} type="file" accept=".json,.alur-story.json,application/json" className="hidden" onChange={handleCompareImport} /><button type="button" onClick={() => setShareOpen(true)} className="pressable flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-3 text-[11px] font-bold text-slate-700"><Share2 className="h-3.5 w-3.5" /> Share</button></>}<button type="button" onClick={() => setPresenting(!presenting)} className="pressable flex h-8 items-center gap-1.5 rounded-md bg-slate-900 px-3 text-[11px] font-bold text-white"><MonitorPlay className="h-3.5 w-3.5" />{presenting ? 'Exit presentation' : 'Present'}</button></div></div>{!presenting && <input value={explain.audience || ''} onChange={(event) => updateDocument({ audience: event.target.value })} placeholder="Audience or decision owner (optional)" className="mt-2 w-full border-t border-slate-100 pt-2 text-[11px] text-slate-600 outline-none" aria-label="Explanation audience" />}</header>
        ...
        {!presenting && <button type="button" onClick={() => addSection({ id: `section-${Date.now()}`, title: 'New section', purpose: 'Describe the role of this section in the argument.' })} className="pressable mt-8 flex items-center gap-1.5 rounded-lg border border-dashed border-slate-300 px-3 py-2 text-xs font-bold text-slate-600"><Plus className="h-3.5 w-3.5" /> Add section</button>}
        {explain.cards.some((card) => card.provenance?.caveats.length) && <div className="mt-8 flex items-start gap-2 rounded-lg bg-amber-50 p-3 text-[11px] text-amber-900"><AlertCircle className="h-3.5 w-3.5 shrink-0" /> Frozen evidence retains caveats and source fingerprints. Refresh is always explicit.</div>}
      </div>
    </main>
    ...
    <StoryExportDialog open={shareOpen} onClose={() => setShareOpen(false)} />
