...
const ComparisonEvidence = ({ result, card }: { result: ComparisonResult; card: ExplainCard }) => {
  ...
  if (spec && card.comparisonView === 'map') return <ComparisonMapEvidence spec={spec} result={result} differenceEligible={spec.operands.length === 2 && spec.alignment.mode === 'entity-keyed' && Boolean(result.spatialSamples?.length === 2 && result.alignedRecords?.length)} selectedKey={selectedKey} onSelectKey={setSelectedKey} mode={mapMode} onModeChange={setMapMode} />;
...
const SortableCard = ({ card, presenting }: { card: ExplainCard; presenting: boolean }) => {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn('relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm', widthClass[card.width], heightClass[card.height], !presenting && 'pt-12', presenting && card.behaviour === 'live' && 'pt-9', isDragging && 'z-20 opacity-70 shadow-xl')}>
    {!presenting && <div className="absolute right-2 top-2 z-10 flex rounded-md border border-slate-100 bg-white/95 p-0.5 shadow-sm"><button type="button" {...attributes} {...listeners} className="cursor-grab rounded p-1.5 text-slate-500 hover:bg-slate-50" aria-label={`Reorder ${card.title || card.kind}`}><GripVertical className="h-3.5 w-3.5" /></button>{card.provenance && <button type="button" onClick={() => void refresh()} disabled={sourceMissing} className="rounded p-1.5 text-slate-500 hover:bg-slate-50 disabled:opacity-30" aria-label="Refresh frozen evidence"><RefreshCw className="h-3.5 w-3.5" /></button>}<button type="button" onClick={() => update(card.id, { behaviour: card.behaviour === 'frozen' ? 'live' : 'frozen' })} className="rounded px-1.5 text-[9px] font-bold text-slate-600 hover:bg-slate-50" aria-label={`Make card ${card.behaviour === 'frozen' ? 'live' : 'frozen'}`}>{card.behaviour === 'frozen' ? 'Live' : 'Freeze'}</button><button type="button" onClick={() => update(card.id, { width: widths[Math.min(widths.length - 1, currentWidth + 1)] })} className="rounded p-1.5 text-slate-500 hover:bg-slate-50" aria-label="Widen card"><Maximize2 className="h-3.5 w-3.5" /></button><button type="button" onClick={() => update(card.id, { width: widths[Math.max(0, currentWidth - 1)] })} className="rounded p-1.5 text-slate-500 hover:bg-slate-50" aria-label="Narrow card"><Minimize2 className="h-3.5 w-3.5" /></button><button type="button" onClick={() => remove(card.id)} className="rounded p-1.5 text-slate-500 hover:bg-rose-50 hover:text-rose-600" aria-label="Remove card"><Trash2 className="h-3.5 w-3.5" /></button></div>}
