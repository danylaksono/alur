        ...
        {lineage.rows.map((row) => (
          <li key={row.id} style={{ marginLeft: `${Math.min(row.depth, 4) * 14}px` }} className={cn(row.depth > 0 && 'border-l-2 border-slate-200 pl-3')}>
            <div className="flex flex-wrap items-baseline gap-x-2">
              {row.depth > 0 && <GitBranch className="h-3 w-3 shrink-0 text-slate-500" />}
              <span className="text-xs font-bold text-slate-800">{row.name}</span>
              <span className={cn('text-[11px] font-semibold uppercase tracking-wide', row.hasResult ? 'text-emerald-600' : 'text-slate-500')}>
              ...
              row.differences.length ? (
                <ul className="mt-1 space-y-0.5">
                  {row.differences.map((difference) => (
                    <li key={difference.path} className="text-[11px] leading-4 text-slate-600">
                      <span className="font-mono text-[11px] text-slate-500">{difference.path}</span>
                      {' '}
                      <span className="text-slate-500">{formatVariantValue(difference.before)}</span>
                      {' → '}
                ...
                <p className="mt-1 text-[11px] leading-4 text-slate-500">Branched but not yet changed from its parent.</p>
              )
            )}
            ...
            {row.assumptions.length > 0 && (
              <ul className="mt-1.5 space-y-0.5">
                {row.assumptions.map((assumption) => (
                  <li key={assumption} className="text-[11px] leading-4 text-slate-500">· {assumption}</li>
                ))}
              </ul>
            )}
          </li>
