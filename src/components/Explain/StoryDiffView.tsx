...
const ClaimSide = ({ card, label, colour }: { card?: ExplainCard; label: string; colour: string }) => (
  <div className="min-w-0 flex-1">
    <p className={cn('text-[11px] font-bold uppercase tracking-wider', colour)}>{label}</p>
    {card ? (
      <>
        ...
        {card.caveat && <p className="mt-1.5 text-[11px] leading-4 text-amber-800">Caveat: {card.caveat}</p>}
      </>
    ) : (
      <p className="mt-1 text-xs italic text-slate-500">No matching claim</p>
    )}
  </div>
);
  ...
  return (
    <article className={cn('rounded-xl border p-4', style.className)}>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className={cn('flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide', style.badge)}>
          <Icon className="h-3 w-3" />{style.label}
        </span>
        {claim.similarity > 0 && (
          <span className="text-[11px] text-slate-500">matched on wording · {Math.round(claim.similarity * 100)}% overlap</span>
        )}
        {claim.reasons.map((reason) => (
          <span key={reason} className="rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-900" title={REASON_DETAIL[reason]}>
            {REASON_LABEL[reason]}
          </span>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:gap-6">
        ...
        <ArrowLeftRight className="hidden h-3.5 w-3.5 shrink-0 self-center text-slate-400 md:block" aria-hidden="true" />
        <ClaimSide card={claim.right} label="Their finding" colour="text-violet-700" />
      </div>

      {claim.status === 'conflict' && claim.reasons.length > 0 && (
        <p className="mt-3 rounded-lg bg-white/70 px-3 py-2 text-[11px] leading-4 text-slate-700">
          These may not actually disagree — {claim.reasons.map((reason) => REASON_DETAIL[reason].toLowerCase()).join(' ')}
        </p>
      )}
      {claim.status === 'conflict' && claim.reasons.length === 0 && (
        <p className="mt-3 rounded-lg bg-white/70 px-3 py-2 text-[11px] leading-4 text-rose-900">
        ...
        <div className="mx-auto max-w-5xl p-4 md:p-7">
          <div className="grid gap-3 md:grid-cols-2">
            <section className="rounded-xl border border-blue-200 bg-white p-4">
              <p className="text-[11px] font-bold uppercase tracking-wider text-blue-700">Yours</p>
              <h2 className="mt-1 truncate text-sm font-bold text-slate-900">{left.title}</h2>
              <p className="mt-1 text-[11px] text-slate-500">{left.author || 'Unattributed'}</p>
            </section>
            <section className="rounded-xl border border-violet-200 bg-white p-4">
              <p className="text-[11px] font-bold uppercase tracking-wider text-violet-700">Theirs</p>
              <h2 className="mt-1 truncate text-sm font-bold text-slate-900">{right.title}</h2>
              <p className="mt-1 text-[11px] text-slate-500">{right.author || 'Unattributed'}</p>
            </section>
          </div>
          ...
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-2 grid gap-2 text-[11px] md:grid-cols-3">
              <p className="text-slate-700"><strong className="block text-[11px] uppercase tracking-wide text-slate-500">In common</strong>{diff.sharedSources.join(', ') || 'None'}</p>
              <p className="text-slate-700"><strong className="block text-[11px] uppercase tracking-wide text-blue-700">Only yours</strong>{diff.leftOnlySources.join(', ') || 'None'}</p>
              <p className="text-slate-700"><strong className="block text-[11px] uppercase tracking-wide text-violet-700">Only theirs</strong>{diff.rightOnlySources.join(', ') || 'None'}</p>
            </div>
          </section>
          ...
          <p className="mt-8 border-t border-slate-200 pt-4 text-[11px] leading-5 text-slate-500">
