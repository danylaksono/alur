...
export const Header = () => {
  ...
  const [importedProject, setImportedProject] =
  ...
  const undoLabel =
    analysisHistory.past[analysisHistory.past.length - 1]?.label;
  ...
  const handleUrlImport = async () => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message: error?.message || "Could not import this URL.",
      });
    } finally {
  ...
  const handleClipboardImport = async () => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message: error?.message || "Could not import the pasted JSON.",
      });
    } finally {
      setRemoteLoading(false);
    }
  ...
  const handleNewProject = () => {
    if (
      !hasWork ||
      window.confirm(
        "Clear the current workspace? Loaded data, workflow nodes, and layers will be removed.",
      )
    ) {
      resetWorkspace();
    }
  ...
  const handleProjectImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      hasWork &&
      !window.confirm(
        "Replace the current workspace with this project? Loaded data and unsaved changes will be removed.",
      )
    )
      return;
    try {
      ...
      const { restored, missing } = await restoreSourcesFromCache(
        declared,
        manifest,
      );
      ...
      addToast({
        type: "error",
        message: `Could not open project: ${error?.message || "Unknown error"}`,
      });
    }
  };
  ...
  const handleStoryImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast({
        type: "error",
        message: `Could not open story: ${error?.message || "Unknown error"}`,
      });
    }
  };
  ...
  const handleRelink = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!sourceMatchesFile(source, file)) {
      addToast({
        type: "error",
        message: `This file does not match ${source.name}. Choose the original file with the same name${source.size === undefined ? "" : " and size"}.`,
      });
      return;
    }
    ...
    const remaining = missingSources.filter(
  ...
  return (
    <header className="z-50 flex h-14 shrink-0 items-center justify-between border-b border-slate-200/80 bg-white px-2 shadow-[0_1px_0_rgba(15,23,42,0.02)] md:px-4">
      <div
        ...
        role="group"
        aria-label="ALUR interactive visual analytics"
      >
        ...
        <div ref={projectMenuRef} className="relative min-w-0">
          {isRenaming ? (
            ...
            <button
              ...
              className="pressable flex h-8 min-w-0 max-w-[9rem] items-center gap-1 rounded-lg px-2 text-[13px] font-semibold text-slate-800 transition-colors hover:bg-slate-100 md:max-w-[16rem]"
              aria-haspopup="menu"
            ...
            >
              <span
                className={cn(
                  "truncate",
                  !projectName && "font-medium text-slate-500",
                )}
              >
          ...
          {isProjectMenuOpen && (
            <div
              ...
              className="absolute left-0 top-9 z-[100] w-52 rounded-lg border border-slate-200 bg-white p-1 shadow-xl"
            >
              <button
                type="button"
                role="menuitem"
                onClick={startRename}
                className="pressable flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-[11px] font-semibold text-slate-600 hover:bg-slate-50"
              >
              ...
              <button
                ...
                disabled={!hasWork}
                onClick={() => {
                  downloadProjectManifest(createProjectManifest());
                  setProjectMenuOpen(false);
                }}
                className="pressable flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-[11px] font-semibold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-35"
              ...
              <button
                ...
                className="pressable flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-[11px] font-semibold text-slate-600 hover:bg-slate-50"
                title="Read a shared explanation — no data needed"
              >
        ...
        {hasWork && (
          <span
            ...
            title={
              recoverySave.savedAt
                ? `Recovery saved ${new Date(recoverySave.savedAt).toLocaleString()}`
                : "Local crash recovery status"
            }
            aria-live="polite"
          >
            {recoverySave.status === "saving"
              ? "Saving…"
              : recoverySave.status === "saved"
                ? "Saved locally"
                : recoverySave.status === "error"
      ...
      <div className="flex items-center gap-2">
        ...
        {!duckdbReady && (
          <span
            className="mr-1 hidden items-center gap-1.5 text-[11px] text-muted-foreground md:flex"
            title="DuckDB engine initializing"
          >
        ...
        <button
          ...
          className="pressable hidden h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-[11px] font-semibold text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-800 lg:flex"
        ...
        <div className="flex items-center overflow-hidden rounded-lg border border-slate-200 bg-white">
          <button
            ...
            disabled={!undoLabel}
            className="pressable flex h-8 w-8 items-center justify-center text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-30"
          ...
          <button
            ...
            disabled={!redoLabel}
            className="pressable flex h-8 w-8 items-center justify-center border-l border-slate-200 text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-30"
        ...
        <input
          ref={projectInputRef}
          type="file"
          accept=".json,.alur.json,application/json"
          className="hidden"
        ...
        <input
          ref={storyInputRef}
          type="file"
          accept=".json,.alur-story.json,application/json"
          className="hidden"
          onChange={handleStoryImport}
        />
        ...
        {missingSources.length > 0 && (
          <button
            ...
            className="pressable flex h-8 items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2.5 text-[11px] font-semibold text-amber-700 hover:bg-amber-100"
        ...
        <button
          ...
          className="pressable flex items-center gap-1.5 rounded-lg bg-primary px-2.5 py-2 text-xs font-semibold text-white transition-colors hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-50 md:px-3 md:py-1.5"
          aria-label="Add data"
        >
        ...
        <button
          ...
          className="pressable rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700"
          title="Settings"
          aria-label="Settings"
        >
      ...
      {isAddDataOpen && (
        <div
          className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/35 p-4"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !isRemoteLoading)
              setAddDataOpen(false);
          }}
        >
          <section
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Parquet, CSV, JSON, and GeoJSON run locally in your browser.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-4 p-5">
              <button
                ...
                className="pressable flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 px-4 py-5 text-xs font-bold text-slate-600 hover:border-sky-300 hover:bg-sky-50 hover:text-sky-700"
              >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2 rounded-lg border border-slate-200 p-3">
                  <label
                    htmlFor="alur-data-url"
                    className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-slate-500"
                  >
                    <Link2 className="h-3.5 w-3.5" /> Data URL
                  </label>
                  <input
                    id="alur-data-url"
                    type="url"
                  ...
                  <button
                    ...
                    disabled={!dataUrl.trim() || isRemoteLoading}
                    className="pressable flex h-8 w-full items-center justify-center gap-1.5 rounded-md bg-slate-900 text-[11px] font-bold text-white disabled:opacity-40"
                  >
                    {isRemoteLoading && (
                ...
                <div className="space-y-2 rounded-lg border border-slate-200 p-3">
                  ...
                  <button
                    ...
                    disabled={!clipboardText.trim() || isRemoteLoading}
                    className="pressable flex h-8 w-full items-center justify-center gap-1.5 rounded-md bg-slate-900 text-[11px] font-bold text-white disabled:opacity-40"
                  >
                    {isRemoteLoading && (
                      <Loader2 className="h-3 w-3 animate-spin" />
      ...
      {isRelinkOpen && missingSources.length > 0 && (
        <div
          className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/35 p-4"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setRelinkOpen(false);
          }}
        >
          <section
            role="dialog"
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Project files intentionally contain no source records. Choose
                  each original file to restore tables and map layers.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="max-h-80 space-y-2 overflow-y-auto p-4">
              {missingSources.map((source) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => chooseRelinkFile(source)}
                    className="pressable shrink-0 rounded-md bg-slate-900 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-slate-700"
                  >
                    Choose file
                  </button>
                </div>
            ...
            <div className="flex justify-end border-t bg-slate-50 px-4 py-3">
              <button
                type="button"
                onClick={() => setRelinkOpen(false)}
                className="pressable rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100"
              >
                Relink later
              </button>
            </div>
