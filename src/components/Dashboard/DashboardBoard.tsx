  ...
  return (
    <article className={`relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm ${card.width === 2 ? 'lg:col-span-2' : ''} ${heightClass(card.height)}`}>
      {!presenting && <div className="absolute right-2 top-2 z-10 flex items-center gap-1 rounded-md bg-white/90 p-0.5 shadow-sm">
        <button type="button" onClick={() => update(card.id, { width: card.width === 1 ? 2 : 1 })} className="pressable rounded p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700" aria-label={`${card.width === 1 ? 'Widen' : 'Narrow'} ${card.title || card.kind} card`}>{card.width === 1 ? <Maximize2 className="h-3.5 w-3.5" /> : <Minimize2 className="h-3.5 w-3.5" />}</button>
        <button type="button" onClick={() => remove(card.id)} className="pressable rounded p-1.5 text-slate-500 hover:bg-rose-50 hover:text-rose-600" aria-label={`Remove ${card.title || card.kind} card`}><Trash2 className="h-3.5 w-3.5" /></button>
      </div>}
      {children}
    </article>
  );
...
const MiniChart = ({ card }: { card: DashboardCard }) => {
  ...
  if (!chart) return <p className="text-xs text-slate-500">The referenced chart is no longer available.</p>;
  ...
  return <div>
    ...
    <p className="mt-1 text-[11px] uppercase tracking-wide text-slate-500">{chart.aggregation} · {chart.dimensionField}</p>
    {['line', 'area', 'scatter'].includes(chart.type) ? <p className="mt-8 text-xs text-slate-500">Interactive {chart.type} chart · open Charts to explore and brush.</p> : error ? <p className="mt-8 text-xs text-rose-500">Chart query failed.</p> : !result ? <p className="mt-8 text-xs text-slate-500" role="status">Loading chart…</p> : <div className="mt-4 space-y-2" role="img" aria-label={`${chart.title}. ${result.filteredRows} active of ${result.totalRows} rows.`}>{result.data.slice(0, 8).map((datum) => <div key={datum.key} className="grid grid-cols-[minmax(5rem,1fr)_3fr_3rem] items-center gap-2 text-[11px]"><span className="truncate font-semibold text-slate-600">{datum.label}</span><span className="h-2 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full" style={{ width: `${datum.value / max * 100}%`, backgroundColor: datum.color }} /></span><span className="text-right tabular-nums text-slate-500">{datum.value.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span></div>)}</div>}
  </div>;
};
...
const MiniKpi = ({ card }: { card: DashboardCard }) => {
  ...
  if (!spec) return <p className="text-xs text-slate-500">The referenced metric is no longer available.</p>;
  return <div className="flex h-full flex-col justify-between"><div><Gauge className="h-5 w-5 text-sky-600" /><h2 className="mt-3 pr-16 text-xs font-bold uppercase tracking-wide text-slate-500">{card.title || spec.title}</h2></div><div className="mt-4 text-4xl font-black tabular-nums text-slate-900">{result?.value === null || result?.value === undefined ? '—' : result.value.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div><p className="mt-2 text-[11px] text-slate-500">{result ? `${result.activeRows.toLocaleString()} active / ${result.totalRows.toLocaleString()} total rows` : 'Calculating…'}</p></div>;
};
...
const DatasetCard = ({ card, dataset }: { card: DashboardCard; dataset?: DatasetDescriptor }) => {
  ...
  if (!dataset) return <p className="text-xs text-slate-500">The referenced dataset is no longer available.</p>;
  return <div><Database className="h-5 w-5 text-teal-600" /><h2 className="mt-3 pr-16 text-sm font-extrabold text-slate-800">{card.title || dataset.name}</h2><p className="mt-1 text-xs text-slate-500">{dataset.rowCount?.toLocaleString() || 'Unknown'} rows · {dataset.fields.length} fields</p><div className="mt-4 flex flex-wrap gap-1">{dataset.fields.slice(0, 8).map((field) => <span key={field.name} className="rounded bg-slate-100 px-2 py-1 text-[11px] text-slate-500">{field.name}</span>)}</div><button type="button" onClick={() => { if (dataset.source.kind === 'layer') selectLayer(dataset.id); else if (dataset.source.kind === 'workflow-node') setSelectedNodeId(dataset.source.nodeId); setWorkspaceMode('explore'); openDrawerTab('table'); }} className="pressable mt-5 rounded-md bg-slate-900 px-3 py-2 text-[11px] font-bold text-white">Explore table</button></div>;
};
...
export const DashboardBoard = () => {
  ...
  return <>
    <header className="col-span-full flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      ...
      {!presenting && <div className="flex items-center gap-1"><span className="mr-1 text-[11px] font-bold uppercase tracking-wide text-slate-500">Add</span>{(['chart', 'kpi', 'table', 'note'] as const).map((kind) => <button key={kind} type="button" onClick={() => addCard(kind)} disabled={(kind === 'chart' && !charts.length) || (kind === 'kpi' && !kpis.length) || (kind === 'table' && !datasets.length)} className="pressable flex h-8 items-center gap-1 rounded-md border border-slate-200 px-2 text-[11px] font-bold capitalize text-slate-600 hover:bg-slate-50 disabled:opacity-30"><Plus className="h-3 w-3" />{kind}</button>)}</div>}
      {bookmarks.length > 0 && <select defaultValue="" onChange={(event) => { if (event.target.value) restoreBookmark(event.target.value); }} aria-label="Apply bookmark presentation state" className="h-8 rounded-md border border-slate-200 bg-white px-2 text-[11px] text-slate-600"><option value="">Presentation state…</option>{bookmarks.map((bookmark) => <option key={bookmark.id} value={bookmark.id}>{bookmark.name}</option>)}</select>}
      <button type="button" onClick={() => setPresenting(!presenting)} className="pressable flex h-8 items-center gap-1.5 rounded-md bg-slate-900 px-3 text-[11px] font-bold text-white"><MonitorPlay className="h-3.5 w-3.5" />{presenting ? 'Edit board' : 'Present'}</button>
      {activeFilters.length > 0 && <div className="basis-full border-t border-slate-100 pt-2" aria-label="Shared dashboard filters"><span className="mr-2 text-[11px] font-bold uppercase tracking-wide text-slate-500">Shared filters</span>{activeFilters.map(({ datasetId, filter }, index) => <span key={`${datasetId}-${filter.field}-${index}`} className="mr-1 inline-flex rounded-full bg-sky-50 px-2 py-1 text-[11px] font-semibold text-sky-700">{datasets.find((item) => item.id === datasetId)?.name || datasetId}: {filter.field}</span>)}</div>}
    </header>
