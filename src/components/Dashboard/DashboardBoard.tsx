  ...
  return (
    <article className={`relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm ${card.width === 2 ? 'lg:col-span-2' : ''} ${heightClass(card.height)}`}>
      {!presenting && <div className="absolute right-2 top-2 z-10 flex items-center gap-1 rounded-md bg-white/90 p-0.5 shadow-sm">
        <button type="button" onClick={() => update(card.id, { width: card.width === 1 ? 2 : 1 })} className="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label={`${card.width === 1 ? 'Widen' : 'Narrow'} ${card.title || card.kind} card`}>{card.width === 1 ? <Maximize2 className="h-3.5 w-3.5" /> : <Minimize2 className="h-3.5 w-3.5" />}</button>
        <button type="button" onClick={() => remove(card.id)} className="rounded p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-600" aria-label={`Remove ${card.title || card.kind} card`}><Trash2 className="h-3.5 w-3.5" /></button>
      </div>}
      {children}
    </article>
  );
...
export const DashboardBoard = () => {
  ...
  return <>
    <header className="col-span-full flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      ...
      {!presenting && <div className="flex items-center gap-1"><span className="mr-1 text-[10px] font-bold uppercase tracking-wide text-slate-400">Add</span>{(['chart', 'kpi', 'table', 'note'] as const).map((kind) => <button key={kind} type="button" onClick={() => addCard(kind)} disabled={(kind === 'chart' && !charts.length) || (kind === 'kpi' && !kpis.length) || (kind === 'table' && !datasets.length)} className="flex h-8 items-center gap-1 rounded-md border border-slate-200 px-2 text-[10px] font-bold capitalize text-slate-600 hover:bg-slate-50 disabled:opacity-30"><Plus className="h-3 w-3" />{kind}</button>)}</div>}
      ...
      <button type="button" onClick={() => setPresenting(!presenting)} className="flex h-8 items-center gap-1.5 rounded-md bg-slate-900 px-3 text-[10px] font-bold text-white"><MonitorPlay className="h-3.5 w-3.5" />{presenting ? 'Edit board' : 'Present'}</button>
