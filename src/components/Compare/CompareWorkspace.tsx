...
import { AlertTriangle, BarChart3, Check, Copy, Database, GitCompareArrows, Map, Plus, Save, Trash2, X } from 'lucide-react';
...
import { PatternGuide } from '../Patterns/PatternGuide';
...
export const CompareWorkspace = () => {
  ...
  const addOperand = () => {
    ...
    setDraft((current) => ({ ...current, operands: [...current.operands, { id: `operand-${Date.now()}`, label: `Operand ${index + 1}`, colour: COLOURS[index], datasetId: dataset.id, scope: { kind: 'whole-dataset' } }], updatedAt: Date.now() }));
  };
  const save = () => {
    ...
    addToast({ type: 'success', message: 'Comparison saved with its current operand definitions.' });
  };
  const pin = () => {
    if (!result) return;
    const sourceVersions = Object.fromEntries(draft.operands.map((operand) => [operand.datasetId, datasets[operand.datasetId]?.sourceUpdatedAt]));
    ...
    addExplainCard({
      ...
      title: draft.name, width: 12, height: 'standard', behaviour: 'frozen', frozenValues: structuredClone(result),
    ...
    addToast({ type: 'success', message: 'Frozen comparison evidence pinned to Explain.' });
  };
  ...
  return <div className="flex min-h-0 flex-1 overflow-hidden bg-slate-50">
    {configOpen && <aside className="w-[340px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white p-4 pb-20 max-lg:w-[300px] max-md:absolute max-md:z-20 max-md:h-full max-md:w-[min(88vw,340px)]">
      ...
      <div className="mt-5 flex items-center justify-between"><h3 className="text-xs font-bold text-slate-800">Operands <span className="font-normal text-slate-600">({draft.operands.length}/4)</span></h3><button type="button" disabled={draft.operands.length >= 4 || !datasetList.length} onClick={addOperand} className="text-[11px] font-bold text-blue-600 disabled:text-slate-300">+ Add</button></div>
      <div className="mt-2 space-y-3">{draft.operands.map((operand, index) => <div key={operand.id} className="rounded-xl border border-slate-200 p-3" style={{ borderLeftColor: operand.colour, borderLeftWidth: 4 }}>
        <div className="flex gap-2"><input value={operand.label} onChange={(event) => updateOperand(operand.id, { label: event.target.value })} className="min-w-0 flex-1 border-0 p-0 text-xs font-bold text-slate-800 outline-none" aria-label={`Operand ${index + 1} label`} /><button type="button" disabled={draft.operands.length <= 2} onClick={() => setDraft({ ...draft, operands: draft.operands.filter((item) => item.id !== operand.id) })} className="text-slate-300 hover:text-rose-500 disabled:opacity-20" aria-label={`Remove ${operand.label}`}><Trash2 className="h-3.5 w-3.5" /></button></div>
    ...
    <main className="min-w-0 flex-1 overflow-y-auto p-5 md:p-6">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-5 flex gap-1 overflow-x-auto border-b border-slate-200" role="tablist">{VIEW_LABELS.map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={view === id} onClick={() => setView(id)} className={cn('border-b-2 px-3 py-2 text-xs font-bold', view === id ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-800')}>{label}</button>)}</div>
        <div className="mt-5">{loading && !result ? <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">Computing comparison…</div> : !result ? <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center"><Database className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">Configure two to four queryable operands</p><p className="mt-1 text-xs text-slate-600">Workflow outputs and non-spatial tables participate through the dataset registry.</p></div> : view === 'overview' ? <Overview spec={draft} result={result} /> : view === 'distribution' ? <Distribution spec={draft} result={result} /> : view === 'categories' ? <Categories spec={draft} result={result} /> : view === 'time' ? result.temporalSeries.length ? <pre className="rounded-xl border bg-white p-4 text-xs">{JSON.stringify(result.temporalSeries, null, 2)}</pre> : <EmptyView icon={BarChart3} title="Map time fields to compare change" detail="Temporal alignment preserves missing periods instead of joining across gaps." /> : view === 'map' ? <EmptyView icon={Map} title={compatibility.differenceMapEligible ? 'Difference map eligible' : 'Spatial small multiples need compatible data'} detail={compatibility.differenceMapEligible ? 'Exactly two entity-aligned spatial operands and a numeric measure are configured.' : 'Use spatial operands for synchronised maps. A difference map additionally requires exactly two entity-aligned operands.'} /> : <EmptyView icon={Check} title="Record alignment is explicit" detail={draft.alignment.mode === 'aggregate-only' ? 'Choose entity-keyed alignment and map a stable key before record-level deltas are available.' : 'Aligned record preview is bounded and never treated as the comparison denominator.'} />}</div>
      </div>
    </main>
  </div>;
};
