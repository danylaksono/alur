...
import { ComparisonMapEvidence, ComparisonRecordsEvidence, ComparisonTimeEvidence } from './ComparisonEvidenceViews';
  ...
  return <div className="space-y-5">
    {result.summaries.map((summary) => {
      ...
      return <section key={summary.measureId} className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-4 flex items-baseline justify-between gap-3"><h3 className="text-sm font-bold text-slate-800">{measure?.label || summary.measureId}</h3><span className="text-[11px] text-slate-600">Common scale · denominator shown</span></div>
        <div className={cn('grid gap-4', spec.operands.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-2 xl:grid-cols-4')}>
          {summary.values.map((value) => {
            ...
            return <div key={value.operandId} className="min-w-0">
              ...
              <p className="mt-1 text-[11px] text-slate-600">n={value.denominator.toLocaleString()} · {value.missing.toLocaleString()} missing</p>
            </div>;
          })}
        </div>
      </section>;
...
const Distribution = ({ spec, result }: { spec: ComparisonSpec; result: ComparisonResult }) => <div className="grid gap-4 lg:grid-cols-2">
  {result.distributions.map((distribution) => {
    ...
    return <section key={`${distribution.measureId}-${distribution.operandId}`} className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <p className="mt-2 text-[11px] text-slate-500">Normalised share · hover bars for bin counts</p>
    </section>;
  })}
</div>;
...
export const CompareWorkspace = ({ onBack }: { onBack?: () => void }) => {
  ...
  return <div className="flex min-h-0 flex-1 overflow-hidden bg-slate-50">
    {configOpen && <aside className="w-[340px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white p-4 pb-20 max-lg:w-[300px] max-md:absolute max-md:z-20 max-md:h-full max-md:w-[min(88vw,340px)]">
      <div className="mb-4 flex items-center justify-between"><div>{onBack && <button type="button" onClick={onBack} className="pressable mb-2 flex items-center gap-1 text-[11px] font-bold text-slate-500 hover:text-blue-700"><ArrowLeft className="h-3 w-3" /> All analysis questions</button>}<p className="text-[11px] font-bold uppercase tracking-[.16em] text-blue-600">Compare</p><h2 className="text-base font-bold text-slate-900">Comparison sessions</h2></div><div className="flex gap-1"><button type="button" className="pressable rounded-lg border p-2 text-slate-500 hover:bg-slate-50" onClick={() => setDraft(defaultComparison(datasetList[0]?.id, datasetList[0]?.name))} aria-label="New comparison"><Plus className="h-4 w-4" /></button><button type="button" onClick={() => setConfigOpen(false)} className="pressable rounded-lg border p-2 text-slate-500 md:hidden" aria-label="Close comparison configuration"><X className="h-4 w-4" /></button></div></div>
      ...
      <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500">Name<input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value, updatedAt: Date.now() })} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold normal-case tracking-normal text-slate-800" /></label>
      <div className="mt-5 flex items-center justify-between"><h3 className="text-xs font-bold text-slate-800">Groups <span className="font-normal text-slate-600">({draft.operands.length}/4)</span></h3><button type="button" disabled={draft.operands.length >= 4 || !datasetList.length} onClick={addOperand} className="pressable text-[11px] font-bold text-blue-600 disabled:text-slate-400">+ Add</button></div>
