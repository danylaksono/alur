...
export const CompareWorkspace = ({ onBack }: { onBack?: () => void }) => {
  ...
  return <div className="flex min-h-0 flex-1 overflow-hidden bg-slate-50">
    {configOpen && <aside className="w-[340px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white p-4 pb-20 max-lg:w-[300px] max-md:absolute max-md:z-20 max-md:h-full max-md:w-[min(88vw,340px)]">
      ...
      <div className="mt-5 flex items-center justify-between"><h3 className="text-xs font-bold text-slate-800">Groups <span className="font-normal text-slate-600">({draft.operands.length}/4)</span></h3><button type="button" disabled={draft.operands.length >= 4 || !datasetList.length} onClick={addOperand} className="pressable text-[11px] font-bold text-blue-600 disabled:text-slate-300">+ Add</button></div>
