...
import {
  queryLayerFieldProfile,
  queryLayerTemporalRange,
  type TemporalRange,
...
const DistributionPreview = ({
        ...
        {profile.bins.map((bin) => (
          ...
          >
            <button
              ...
              style={{
                height: `${Math.max(4, (bin.count / maxCount) * 60)}px`,
              }}
  ...
  const maxCount = Math.max(
    ...profile.categories.map((category) => category.count),
    1,
  );
  return (
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-100">
              <div
                ...
                style={{
                  width: `${Math.max(4, (category.count / maxCount) * 100)}%`,
                }}
              />
            </div>
          </div>
...
const temporalFieldsForLayer = (layer: MapLayer | null) => {
  ...
  return layer.source.fields
    .filter(
      (field) =>
        TEMPORAL_TYPE_PATTERN.test(field.type) ||
        TEMPORAL_NAME_PATTERN.test(field.name),
    )
    .map((field) => field.name)
    .filter(
      (name) =>
        !excludedFields.has(name.toLowerCase()) &&
        !name.toLowerCase().startsWith("__alur_"),
    );
};
...
export const VisualisationPanel = ({
  ...
  const isRestyling = useStore((s) =>
    Boolean(selectedLayer && s.restylingLayerIds[selectedLayer.id]),
  );

  const fields = useMemo(
    () => fieldNamesForLayer(selectedLayer),
    [selectedLayer?.id, selectedLayer?.source],
  );
  const temporalCandidates = useMemo(
    () => temporalFieldsForLayer(selectedLayer),
    [selectedLayer?.id, selectedLayer?.source],
  );
  const fieldsKey = fields.join("|");
  const geometryKind = useMemo(
    () => (selectedLayer ? geometryKindForLayer(selectedLayer) : null),
    [selectedLayer],
  );
  ...
  const [bivariatePaletteId, setBivariatePaletteId] = useState(
    BIVARIATE_PALETTES[0].id,
  );
  ...
  const [hexbinMethod, setHexbinMethod] = useState<{
    method: HexbinMethod;
    resolution?: number;
  ...
  const [temporalRange, setTemporalRange] = useState<TemporalRange | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    queryLayerTemporalRange({ layer: selectedLayer, column: temporalField })
      .then((result) => {
  ...
  useEffect(() => {
    ...
    resolveH3CellColumn(selectedLayer)
      .then((column) => {
        if (!cancelled) setH3CellColumn(column);
      })
      .catch(() => {
        if (!cancelled) setH3CellColumn(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  const hasActiveStyle = Boolean(
    selectedLayer?.visualisation || selectedLayer?.legend,
  );
  ...
  useEffect(() => {
    ...
    if (kind === "glyph_grid") {
      if (!canApply) return;
      const multivariate = ["pie", "donut", "bars", "radial"].includes(
        glyphType,
      );
    ...
    if (kind === "dot_density" && profile.kind === "numeric") {
      ...
      generateDotDensityGeoJSON(
        { id: selectedLayer.id, geojson: selectedLayer.geojson },
        {
          field,
          dotValue: visualisation.dotValue,
          color: visualisation.color,
          radius: visualisation.radius,
    ...
    if (kind === "graduated_symbol" && profile.kind === "numeric") {
      const visualisation = buildGraduatedSymbolVisualisation({
        field,
        profile,
      });
      updateLayerVisualisation(
    ...
    if (kind === "hexbin") {
      const visualisation = buildHexbinVisualisation({
        field,
      ...
      generateHexbins(selectedLayer, {
        cellSize: visualisation.cellSize,
        aggregate: hexAggregate,
        field: visualisation.field,
      })
        .then(
          ({
            featureCollection: hexGeojson,
            method,
            resolution,
            cellEdgeMetres,
          }) => {
            ...
            const valueProfile = profileGeoJsonField(
              hexGeojson.features,
              "value",
            );
            const hexVis =
              valueProfile.kind === "numeric"
                ? buildChoroplethVisualisation({
                    field: "value",
        ...
        .catch((err: any) => {
          ...
          addToast({
            type: "error",
            message: `Hexbin failed: ${err?.message || err}`,
          });
        })
    ...
    profileY,
    updateLayerVisualisation,
    clearLayerVisualisation,
  ]);

  const activeFilters = selectedLayer
    ? visualAnalytics.datasets[selectedLayer.id]?.filters || []
    : [];
  ...
  const toggleDistributionFilter = (
    ...
    setLayerFilters(
      selectedLayer.id,
      toggleFilterIn(existingFilters, nextFilter),
    );
  };

  const applyTemporalFilter = () => {
    if (!selectedLayer || !temporalField || (!temporalStart && !temporalEnd))
      return;
    const nextFilter = {
    ...
    const existingFilters =
      visualAnalytics.datasets[selectedLayer.id]?.filters || [];
    const withoutSameField = existingFilters.filter(
      (filter) =>
        !(filter.kind === "temporal" && filter.field === temporalField),
    );
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {selectedLayer?.visualisation && (
            <button
              ...
              onClick={() => {
                if (!selectedLayer) return;
                if (selectedLayer.dotDensityLayerId)
                  removeMapLayer(selectedLayer.dotDensityLayerId);
                if (selectedLayer.hexbinLayerId)
                  removeMapLayer(selectedLayer.hexbinLayerId);
                if (
                  selectedLayer.dotDensityLayerId ||
                  selectedLayer.hexbinLayerId
                ) {
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="space-y-1">
                ...
                >
                  ...
                  {geometryKind === "polygon" && selectedLayer.geojson && (
                    <option value="dot_density">Dot density</option>
                  )}
                  {geometryKind === "line" && (
                    <option value="graduated_line">Line width</option>
                  )}
                ...
                {kind === "extrusion" && (
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setHeightMultiplier(Number(event.target.value) || 1)
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    />
                  </label>
                ...
                {kind === "hexbin" && (
                  <label className="space-y-1">
                    ...
                    <select
                      value={hexCellSize}
                      onChange={(event) =>
                        setHexCellSize(Number(event.target.value))
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    >
                ...
                {kind === "hexbin" && (
                  <label className="space-y-1">
                    ...
                    <select
                      value={hexAggregate}
                      onChange={(event) =>
                        setHexAggregate(event.target.value as HexbinAggregate)
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    >
                ...
                {kind === "h3grid" && h3Extruded && (
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setH3ElevationScale(Number(event.target.value) || 1)
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    />
                  </label>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <label className="space-y-1">
                    ...
                    <select
                      value={glyphMode}
                      onChange={(event) =>
                        setGlyphMode(event.target.value as "grid" | "hex")
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    >
                  ...
                  <label className="space-y-1">
                    ...
                    <select
                      value={glyphType}
                      onChange={(event) =>
                        setGlyphType(event.target.value as GlyphGridGlyph)
                      }
                      className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                    >
                  ...
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      value={glyphCellSize}
                      onChange={(event) =>
                        setGlyphCellSize(Number(event.target.value))
                      }
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <label className="space-y-1">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setGlyphAggregate(
                            event.target.value as "count" | "sum" | "avg",
                          )
                        }
                        className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                      >
                    ...
                    <div className="max-h-40 space-y-0.5 overflow-y-auto rounded-md border border-slate-200 bg-white p-1.5">
                      {fields.map((name) => {
                        ...
                        return (
                          <label
                            key={name}
                            className="flex cursor-pointer items-center gap-2 rounded px-1 py-0.5 text-[11px] text-slate-600 hover:bg-slate-50"
                          >
                            <input
                              type="checkbox"
                              checked={checked}
              ...
              <div className="grid grid-cols-[1fr_1fr_auto] items-end gap-2">
                ...
                <label className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setBivariatePaletteId(event.target.value)
                    }
                    className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                  >
                  ...
                  {[2, 1, 0].flatMap((row) =>
                    [0, 1, 2].map((column) => (
                      <span
                        ...
                        style={{
                          backgroundColor:
                            getBivariatePalette(bivariatePaletteId).colors[
                              row * 3 + column
                            ],
                        }}
                      />
                ...
                {hexbinMethod.method === "h3"
                  ? `Equal-area H3 cells at resolution ${hexbinMethod.resolution}${
                      hexbinMethod.cellEdgeMetres
                        ? ` (about ${
                            hexbinMethod.cellEdgeMetres >= 1000
                              ? `${(hexbinMethod.cellEdgeMetres / 1000).toFixed(1)} km`
                              : `${Math.round(hexbinMethod.cellEdgeMetres)} m`
                          } across)`
                        : ""
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      min={1}
                      value={
                        selectedLayer?.visualisation?.kind === "dot_density"
                          ? selectedLayer.visualisation.dotValue
                          : 100
                      }
                      onChange={(event) => {
                        if (
                        ...
                        const vis = {
                          ...selectedLayer.visualisation,
                          dotValue: Number(event.target.value),
                        };
                        updateLayerVisualisation(
                          selectedLayer.id,
                          vis,
                  ...
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      value={
                        selectedLayer?.visualisation?.kind === "dot_density"
                          ? selectedLayer.visualisation.radius
                          : 2.5
                      }
                      onChange={(event) => {
                        if (
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      return;
                    const dotLayerId = `${selectedLayer.id}-dots`;
                    const existingDots = mapLayers.find(
                      (l) => l.id === dotLayerId,
                    );
                    ...
                    generateDotDensityGeoJSON(
                      ...
                      .then((dotsGeojson) => {
                        addMapLayer({
                          ...
                          color:
                            selectedLayer.visualisation?.kind === "dot_density"
                              ? selectedLayer.visualisation.color
                              : undefined,
                          opacity:
                            selectedLayer.visualisation?.kind === "dot_density"
                              ? selectedLayer.visualisation.opacity
                              : 0.65,
                          visualisation:
                            selectedLayer.visualisation?.kind === "dot_density"
                              ? undefined
                    ...
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          if (!selectedLayer) return;
                          updateMapLayer(selectedLayer.id, {
                            clusterRadius: Number(event.target.value),
                          });
                        }}
                        className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                      />
                    ...
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          if (!selectedLayer) return;
                          updateMapLayer(selectedLayer.id, {
                            clusterMaxZoom: Number(event.target.value),
                          });
                        }}
                        className="h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-[11px] font-semibold text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                      />
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                {temporalRange ? (
                  <TemporalSlider
                    range={temporalRange}
                    field={temporalField}
                    currentFilter={activeFilters.find(
                      (f) => f.kind === "temporal" && f.field === temporalField,
                    )}
                    onApply={(filter) => {
                      if (!selectedLayer) return;
                      const existing =
                      ...
                      const withoutField = existing.filter(
                        (f) =>
                          !(f.kind === "temporal" && f.field === temporalField),
                      );
                      setLayerFilters(selectedLayer.id, [
                        ...withoutField,
                        filter,
                      ]);
                    }}
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        value={temporalStart}
                        onChange={(event) =>
                          setTemporalStart(event.target.value)
                        }
