        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
            <section>
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <label className="flex h-8 w-64 max-w-full items-center gap-2 rounded-md border border-slate-200 bg-white px-2 focus-within:border-sky-400">
                  <Search className="h-3.5 w-3.5 text-slate-500" /><span className="sr-only">Search fields</span><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search fields" className="min-w-0 flex-1 text-xs outline-none" />
                </label>
              </div>
              <div className="overflow-hidden rounded-lg border border-slate-200">
                <div className="grid grid-cols-[minmax(8rem,1fr)_7rem_6rem_7rem_2rem] gap-2 bg-slate-50 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500"><span>Field</span><span>Type</span><span>Missing</span><span>Range / distinct</span><span /></div>
                <div className="max-h-[45vh] divide-y divide-slate-100 overflow-y-auto">
                  {fields.map((field) => {
                    ...
                    return <div key={field.name} className="grid grid-cols-[minmax(8rem,1fr)_7rem_6rem_7rem_2rem] items-center gap-2 px-3 py-2 text-[11px] hover:bg-slate-50">
                      <div className="min-w-0"><div className="truncate font-semibold text-slate-700" title={field.name}>{field.name}</div><div className="text-[11px] capitalize text-slate-500">{field.semanticType}</div></div>
                      <span className="truncate font-mono text-[11px] text-slate-500" title={field.type}>{field.type}</span>
            ...
            <aside className="space-y-3">
              ...
              <p className="text-[11px] leading-relaxed text-slate-500">Signals identify values worth inspecting; unusual data is not necessarily erroneous.</p>
