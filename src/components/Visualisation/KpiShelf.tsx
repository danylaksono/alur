    ...
    <article className="group relative w-56 shrink-0 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm" aria-label={`${spec.title} metric`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <input value={spec.title} onChange={(event) => updateKpi(spec.id, { title: event.target.value })} aria-label="Metric title" className="w-full truncate bg-transparent text-[11px] font-semibold uppercase tracking-wide text-slate-500 outline-none focus-visible:ring-2 focus-visible:ring-sky-400" />
          <div className="mt-0.5 truncate text-[11px] text-slate-500" title={dataset.name}>{dataset.name} · {spec.aggregation}{spec.field ? ` ${spec.field}` : ' rows'}</div>
        </div>
        {loading ? <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-400" /> : <Gauge className="h-3.5 w-3.5 text-sky-500" />}
      </div>
      <div className="mt-1.5 flex items-end justify-between gap-2">
        <div className="truncate text-xl font-extrabold tabular-nums text-slate-800">{error ? 'error' : formatKpiValue(result?.value ?? null, spec)}{spec.unit && spec.format !== 'currency' ? <span className="ml-1 text-[11px] font-semibold text-slate-500">{spec.unit}</span> : null}</div>
        {delta !== null && delta !== undefined && (
          <span className={`rounded px-1 py-0.5 text-[11px] font-bold tabular-nums ${delta > 0 ? 'bg-emerald-50 text-emerald-700' : delta < 0 ? 'bg-rose-50 text-rose-700' : 'bg-slate-100 text-slate-500'}`} title="Difference from unfiltered total">
      ...
      <div className="mt-2 flex items-center gap-1 border-t border-slate-100 pt-1.5">
        ...
        <select id={`kpi-comparison-${spec.id}`} value={spec.comparison} onChange={(event) => updateKpi(spec.id, { comparison: event.target.value as KpiSpec['comparison'] })} className="h-6 min-w-0 flex-1 rounded border border-slate-200 bg-white px-1 text-[11px] text-slate-500 outline-none focus:border-sky-400">
        ...
        <select id={`kpi-format-${spec.id}`} value={spec.format || 'number'} onChange={(event) => updateKpi(spec.id, { format: event.target.value as KpiFormat })} className="h-6 w-16 rounded border border-slate-200 bg-white px-1 text-[11px] text-slate-500 outline-none focus:border-sky-400">
